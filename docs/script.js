document.getElementById('year').textContent = new Date().getFullYear();
  ...
  (function () {
    ...
    var clearProgram;
    var colorProgram;
    var checkerboardProgram;
    var bloomPrefilterProgram;
    var bloomBlurProgram;
    var bloomFinalProgram;
    ...
    var curlProgram;
    var vorticityProgram;
    var pressureProgram;
    var gradientSubtractProgram;
    var displayMaterial;
    ...
    function createTextureAsync (url) {
      ...
      var obj = { texture: texture, width: 1, height: 1, attach: function (id) { gl.activeTexture(gl.TEXTURE0 + id); gl.bindTexture(gl.TEXTURE_2D, texture); return id; } };
      var image = new Image();
    ...
    function createDitheringTexture () {
      ...
      var data = new Uint8Array([
        34,  96, 164, 255,  68, 140, 220, 255,  52,  80, 132, 255,  90, 160, 240, 255,
        88, 128, 212, 255,  40,  72, 124, 255, 110, 170, 248, 255,  60, 104, 176, 255,
        24,  60, 104, 255,  98, 154, 232, 255,  44,  88, 152, 255, 120, 176, 244, 255,
        76, 120, 200, 255,  36,  64, 116, 255, 100, 164, 236, 255,  56,  92, 160, 255
      ]);
    ...
    function scaleByPixelRatio (input) { var pixelRatio = window.devicePixelRatio || 1; return Math.floor(input * pixelRatio); }
    function wrap (value, min, max) { var range = max - min; if (range == 0) return min; return (value - min) % range + min; }
    function hashCode (s) { if (s.length == 0) return 0; var hash = 0; for (var i = 0; i < s.length; i++) { hash = (hash << 5) - hash + s.charCodeAt(i); hash |= 0; } return hash; }
    ...
    function correctDeltaX (delta) { var aspectRatio = canvas.width / canvas.height; if (aspectRatio < 1) delta *= aspectRatio; return delta; }
    function correctDeltaY (delta) { var aspectRatio = canvas.width / canvas.height; if (aspectRatio > 1) delta /= aspectRatio; return delta; }
    ...
    window.addEventListener('pointermove', function (e) {
      ...
      if (heroRect && e.clientY >= heroRect.top && e.clientY <= heroRect.bottom) return;
    ...
    window.addEventListener('pointerup', function () { updatePointerUpData(pointers[0]); }, { passive: true });
    ...
    window.addEventListener('touchmove', function (e) {
      if (activeTouchId === -1) return;
      var touch = getTouchById(e.touches, activeTouchId) || getTouchById(e.changedTouches, activeTouchId);
      if (!touch) return;
      var pointer = pointers[0];
      var posX = scaleByPixelRatio(touch.clientX);
      var posY = scaleByPixelRatio(touch.clientY);
    ...
    updateKeywords();
    initFramebuffers();
    multipleSplats(isMobileDevice ? 2 : parseInt(Math.random() * 20) + 5);
    requestAnimationFrame(update);
  })();
  ...
  (function(){
    ...
    var lines = [
      'academic coordinator @ Digi School, Gandaki Province',
      'teaches AI tools & coding on YouTube',
      'trekker, tinkerer, Arduino hobbyist'
    ];
  ...
  // ============================================================
  // 3D wireframe range — Three.js
  // ============================================================
  (function(){
    var canvas = document.getElementById('heroCanvas');
    var fallback = document.getElementById('ridgeFallback');
    var reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    try {
      ...
      scene.fog = new THREE.FogExp2(0x0e1a26, 0.0026);

      var camera = new THREE.PerspectiveCamera(50, hero.clientWidth / hero.clientHeight, 0.1, 1000);

      var renderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: true, alpha: true });
      renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, isMobileDevice ? 1 : 2));
      renderer.setSize(hero.clientWidth, hero.clientHeight);

      // ---- palette pulled from the page's own CSS variables ----
      var cLow  = new THREE.Color(0x16273a); // shadowed valley (bg-panel)
      var cMid  = new THREE.Color(0x6fa88c); // pine ridge
      var cHigh = new THREE.Color(0xe2a33d); // sunrise amber on peaks

      function hash(x, z){
        var v = Math.sin(x * 12.9898 + z * 78.233) * 43758.5453;
        return v - Math.floor(v);
      }
      function ridgeHeight(x, z){
        var h = 0;
        h += Math.sin(x * 0.021 + z * 0.004) * 16;
        h += Math.sin(x * 0.05 + 1.3) * 8;
        h += Math.cos(x * 0.11 + z * 0.02 + 2.7) * 4;
        h += (hash(x, z) - 0.5) * 6;
        return h;
      }

      var W = 520, D = 420, SEG_X = isMobileDevice ? 64 : 110, SEG_Z = isMobileDevice ? 40 : 70;
      ...
      for(var i = 0; i < pos.count; i++){
        var x = pos.getX(i);
        var z = pos.getZ(i);
        var h = ridgeHeight(x, z);
        pos.setY(i, h);

        var t = (h - minH) / (maxH - minH);
        t = Math.max(0, Math.min(1, t));
        if(t < 0.55){
          tmpColor.copy(cLow).lerp(cMid, t / 0.55);
        } else {
          tmpColor.copy(cMid).lerp(cHigh, (t - 0.55) / 0.45);
        }
      ...
      var range = new THREE.Mesh(geo, mat);
      range.position.z = -60;
      scene.add(range);

      // subtle solid backing so wireframe reads as a mass, not just lines
      var backMat = new THREE.MeshBasicMaterial({ color: 0x0e1a26, transparent: true, opacity: 0.55 });
      var back = new THREE.Mesh(geo.clone(), backMat);
      back.position.copy(range.position);
      back.position.y -= 0.4;
      scene.add(back);

      // ---- starfield ----
      var starCount = isMobileDevice ? 160 : 340;
      var starGeo = new THREE.BufferGeometry();
      var starPos = new Float32Array(starCount * 3);
      for(var s = 0; s < starCount; s++){
        starPos[s*3]     = (Math.random() - 0.5) * 600;
        starPos[s*3 + 1] = Math.random() * 160 + 20;
        starPos[s*3 + 2] = (Math.random() - 0.5) * 500 - 100;
      }
      starGeo.setAttribute('position', new THREE.BufferAttribute(starPos, 3));
      var starMat = new THREE.PointsMaterial({ color: 0xedf1f5, size: 1.1, transparent: true, opacity: 0.5 });
      var stars = new THREE.Points(starGeo, starMat);
      scene.add(stars);

      // ---- camera state, driven by scroll + mouse ----
      var camState  = { x: 0, y: 46, z: 150, lookY: 6 };
      ...
      function animate(){
        ...
        mouseOffset.x += (mouseTarget.x * 9 - mouseOffset.x) * 0.04;
        mouseOffset.y += (mouseTarget.y * 4 - mouseOffset.y) * 0.04;

        camera.position.set(
          camState.x + mouseOffset.x,
          camState.y - mouseOffset.y,
          camState.z
        );
        camera.lookAt(0, camState.lookY, 0);

        range.rotation.y = Math.sin(t * 0.02) * 0.01;
        back.rotation.y = range.rotation.y;
        stars.rotation.y = t * 0.003;

        renderer.render(scene, camera);
      }

      // hide the flat SVG fallback now that WebGL is running
      if(fallback){ fallback.style.display = 'none'; }
      animate();

    } catch(err){
      // WebGL unavailable or failed — flat SVG fallback stays visible
  ...
  // ============================================================
  // GSAP scroll reveals for content sections
  // ============================================================
  (function(){
